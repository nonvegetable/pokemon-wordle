...
def fetch_pokemon_data():
    ...
    # --- FIX: Safely get generation and region ---
    ...
    if generation_info and generation_info.get("name") in regions:
        ...
        region = "an unknown region" # Safe fallback
    ...
    if len(pokemon_data.get('types', [])) > 1:
        hint_pool.append(f"It has two types; one of them is {pokemon_data['types'][0]['type']['name'].capitalize()}.")
    ...
    hint_pool.append(f"It can be found in the {region} region.")
    ...
    return jsonify({
        ...
        "first_hint": f"This Pokémon is a {pokemon_data['types'][0]['type']['name'].capitalize()} type."
    })
...
def user_guess():
    ...
    if user_guess == actual_name:
        ...
        if attempts >= 3:
            return jsonify({
                "correct": False,
                "answer": current_pokemon.get('name', 'Unknown').capitalize()
            })

        response_data = {
            "correct": False,
            "new_hint": current_pokemon.get("hint_list", [])[attempts]
        }
